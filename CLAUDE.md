...
- `src/components/MobileTriptych.svelte` — stepped Home hero for phones, tablets and touch devices: the page is locked (`html.hero-locked`, no document scroll, so the mobile address bar never hides/shows) and GSAP Observer turns each swipe/wheel/arrow key into one step, `tl.tweenTo("rest-i")` on a paused timeline (input ignored while a step plays); the centre panel shows the Young Serif "ABOUT ME" caption (same letters-rise CSS, toggled via `[data-hero-center]` on <html>) and links to About (`client:media="(max-width: 1023px), (hover: none), (pointer: coarse)"`, same query as `.scroll-hero` in global.css): the panels stacked full-height, GSAP ScrollTrigger scrubs grey → colour as each scrolls in and sweeps its label in/out. Shares image variants with the desktop version via `src/lib/triptych-images.ts`.
