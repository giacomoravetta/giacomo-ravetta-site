...
- `src/components/MobileTriptych.svelte` — scroll-driven Home hero for phones, tablets and touch devices: a pinned stage where vertical scroll slides the panels horizontally, native scroll (no hijacking) with ScrollTrigger `snap: "labelsDirectional"` settling on each rest point after a gesture, ~0.35 viewport heights per timeline unit; the centre panel shows the Young Serif "ABOUT ME" caption (same letters-rise CSS, toggled via `[data-hero-center]` on <html>) and links to About (`client:media="(max-width: 1023px), (hover: none), (pointer: coarse)"`, same query as `.scroll-hero` in global.css): the panels stacked full-height, GSAP ScrollTrigger scrubs grey → colour as each scrolls in and sweeps its label in/out. Shares image variants with the desktop version via `src/lib/triptych-images.ts`.
