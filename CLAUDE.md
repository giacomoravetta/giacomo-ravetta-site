...
- Fonts (self-hosted in `public/fonts`, all OFL): Instrument Sans variable (`font-instrument`) for text; Bluu Next Bold (`font-bluu`) for everything "Designer"; Terminal Grotesque (`font-terminal`) for everything "Developer". The hero side words use per-face advance factors (`--adv`) to keep equal length.
