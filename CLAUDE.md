...
- `src/components/Triptych.astro` — pointer Home hero for wide screens with a mouse (hidden under 1024px and on touch devices): three panels of *The Battle of Shijo Nawate* (`src/assets/shijo-nawate/`) scaled as one strip to cover the viewport, desaturated at rest, colour revealed through a canvas brush stroke that follows the pointer; a resting pointer blooms ink over the hovered panel (over the whole painting when resting on the centre). Side words: "designer" letters settle in from a blur (SplitText), "developer" decodes (ScrambleText). The centre shows lorem paragraphs from `homeParagraphs` in `src/i18n/ui.ts` as big centred text; the hero is pinned (ScrollTrigger) and each paragraph rises from the bottom of the panel, holds through the middle and fades out towards the top, scrubbed to the scroll. Panels slide in from below (CSS) once images decode; the brush arms after that.
- `src/components/MobileTriptych.svelte` — scroll-driven Home hero for phones, tablets and touch devices: a pinned stage where vertical scroll slides the panels horizontally (`client:media="(max-width: 1023px), (hover: none), (pointer: coarse)"`, same query as `.scroll-hero` in global.css): the panels stacked full-height, GSAP ScrollTrigger scrubs grey → colour as each scrolls in and sweeps its label in/out. Shares image variants with the desktop version via `src/lib/triptych-images.ts`.
